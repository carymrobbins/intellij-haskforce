...
public class GhcMod {
    ...
    public static String getFlags(@NotNull Project project) {
        Option<SimpleToolSettings> settings = ToolKey.GHC_MOD().getValue(
          PropertiesComponent.getInstance(project)
        );
        return GhcModUtil.changedFlagsIfStack(
          project,
          settings.map(SimpleToolSettings::path).getOrElse(() -> null),
          settings.map(SimpleToolSettings::flags).getOrElse(() -> "")
        );
    }
    ...
    public static class Problem implements HaskellProblem {
        ...
        static {
            fixHandlers = Arrays.asList(
                ...
                Pair.create(
                    Pattern.compile("(-X| )(" + String.join("|", GhcLanguageExtensions.stringArray()) + ")"),
