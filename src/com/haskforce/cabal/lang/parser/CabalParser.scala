...
import com.haskforce.cabal.psi._
...
final class CabalPsiBuilder(builder: PsiBuilder) extends PsiBuilderAdapter(builder) {
  ...
  def parseFieldOrStanza(): Unit = {
    ...
    throw new RuntimeException("Unhandled token: " + getTokenType + ": " + getTokenText)
  }
  ...
  def stanzaNameArg(el: CabalElementType)(): Unit = getTokenType match {
  ...
  def stanza
    ...
    if (!eof() && getTokenType != EOL && getTokenType != LBRACE) {
      error(s"Unexpected $stanzaType argument")
  ...
  def stanzaIndentBody(stanzaType: String, fieldParser: () => Boolean): Unit = {
    if (getTokenType == INDENT) {
      ...
      parseWhile(!break && indent > 0) {
        if (getTokenType == INDENT) throw new RuntimeException("Unexpected INDENT")
        if (!(ifExpr(stanzaType, fieldParser) || fieldParser() || invalidField(stanzaType))) {
          error(s"Unexpected token in $stanzaType stanza: " + getTokenType)
          break = true
        }
  ...
  def funcCall(): Boolean = getTokenType match {
    case _: CabalIdentTokenType =>
      markWith(FUNC_CALL) {
        markWith(FUNC_NAME) {
          if (!getTokenType.isInstanceOf[CabalFuncLikeTokenType]) error("Invalid function")
          advanceLexer()
        }
  ...
  def topLevelField(): Boolean = (
    ...
    || field(UNKNOWN_FIELD, UNKNOWN_KEY, freeform)
    || invalidField("top level")
  )
  ...
  def invalidField(stanzaType: String): Boolean = getTokenType match {
    case _: CabalFieldKeyTokenType =>
      val m = mark()
      error(s"Field not supported in $stanzaType stanza")
      advanceLexer()
      expectColon()
      freeform()
  ...
  def dependency(): Unit = {
    if (!getTokenType.isInstanceOf[CabalIdentTokenType]) {
      throw new AssertionError("Unexpected token: " + getTokenType + ": " + getTokenText)
    }
    val dep = mark()
    ...
    getTokenType match {
      case NUMBERS =>
        val version = mark()
        advanceLexer()
        version.done(DEPENDENCY_VERSION)
