...
class HaskellCompilerToolsForm(moduleBuilder: HaskellModuleBuilder) extends HaskellCompilerToolsFormBase {
  ...
  def save(): Unit = {
    ...
    getGhcModPath.filter(_ => shouldSetGhcModAsDefault).foreach(p => ToolKey.GHC_MOD.PATH.setValue(defaultToolSettings, Option(p).filter(_.nonEmpty)))
    getGhcModiPath.filter(_ => shouldSetGhcModiAsDefault).foreach(p => ToolKey.GHC_MODI.PATH.setValue(defaultToolSettings, Option(p).filter(_.nonEmpty)))
    ...
    moduleBuilder.registerSetupRootModelCallback { rootModel: ModifiableRootModel =>
      ...
      getGhcModPath.foreach(p => ToolKey.GHC_MOD.PATH.setValue(toolSettings, Option(p).filter(_.nonEmpty)))
      getGhcModiPath.foreach(p => ToolKey.GHC_MODI.PATH.setValue(toolSettings, Option(p).filter(_.nonEmpty)))
    }
  }
