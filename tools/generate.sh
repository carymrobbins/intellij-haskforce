...
if ([ ! -f $DIR/.jflex ] || [ ! -f $DIR/.jflex-skeleton ] ||
    ...
    echo "Must set paths in $DIR/.flex, $DIR/.jflex-skeleton, $DIR/.lib, $DIR/.grammar-kit"
    echo "Something like:"
    echo "  $ echo ~/build/idea/tools/lexer/jflex-1.4/bin/jflex > $DIR/.jflex"
    echo "  $ echo ~/build/idea/tools/lexer/idea-flex.skeleton > $DIR/.jflex-skeleton"
    echo "  $ echo ~/Library/Application Support/IdeaIC13/GrammarKit/lib/grammar-kit.jar > $DIR/.grammar-kit"
...
echo_loud "Deleting old gen directory $PROJECT_DIR/gen ..."
